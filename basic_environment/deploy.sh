#!/usr/bin/env bash
#
# deploy.sh — basic_environment: Management + operational Environment, then a LiveSystem into it
#
...
REQUIRED+=(AZURE_TENANT_ID AZURE_MANAGEMENT_SUBSCRIPTION_ID AZURE_OPERATIONAL_SUBSCRIPTION_ID AZURE_SP_CLIENT_ID)
if [[ "${AZURE_CLOUD_AGENT_AUTH:-sp}" == "oidc" ]]; then
  # OIDC: the federated token is minted from the GitHub Actions endpoint.
  REQUIRED+=(ACTIONS_ID_TOKEN_REQUEST_URL ACTIONS_ID_TOKEN_REQUEST_TOKEN)
else
  REQUIRED+=(AZURE_SP_CLIENT_SECRET)
fi
